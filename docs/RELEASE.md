...
TraceSeek-AI 除了 Docker, 还能打成 **Mac/Windows 桌面应用**(双击启动 → 弹出独立应用窗口)。Chromium 直接打进包里, 既当后台抓取引擎, 也用 `--app` 模式渲染控制台窗口; 安装后离线即用。
...
| 平台 | 产物 |
|---|---|
| macOS Apple Silicon | `TraceSeek-AI.dmg` |
| Windows x64 | `TraceSeek-AI-Setup.exe`（Inno Setup 安装程序） |
...
bash packaging/build_macos.sh           # → dist/TraceSeek-AI.app + TraceSeek-AI.dmg
```
...
# → packaging\Output\TraceSeek-AI-Setup.exe
```
...
- 数据存用户目录(macOS `~/Library/Application Support/TraceSeek-AI`、Windows `%APPDATA%\TraceSeek-AI`), 升级/卸载不丢, 与 Docker 版互不干扰。
...
- [`packaging/traceseek.spec`](../packaging/traceseek.spec) — PyInstaller 配置。**故意不让它打 Chromium**(嵌套 .app/权限会让 PyInstaller 报错), 只打 Python + 前端。
