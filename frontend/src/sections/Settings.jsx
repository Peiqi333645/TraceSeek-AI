...
export default function Settings({ status }) {
  ...
  const [billing, setBilling] = useState({ configured: false, active: false, balance: 0 })
  ...
  const activate = async () => {
    ...
    setActivating(true)
    const result = await api.activateCode(activationCode.trim()).catch(() => ({ ok: false, error: '网络连接失败' }))
    setActivationMsg(result.ok ? { ok: true, text: '充值成功' } : { ok: false, text: result.error })
    if (result.ok) {
      setActivationCode('')
  ...
  return (
    <section>
      ...
      <Card className="form-card billing-card">
        <div className="form-title">AI 分析账户</div>
        <p className="form-hint">无需配置 API 或模型。购买次数后输入充值码，即可自动启用智能审核。</p>
        <div className="balance-row">
          <div className="balance-box">
            <span>剩余分析次数</span>
            <b>{billing.offline ? '暂时离线' : Number(billing.balance || 0).toLocaleString('zh-CN')}</b>
          </div>
          <div className="activation-box">
            <input value={activationCode} onChange={(e) => setActivationCode(e.target.value)} placeholder="请输入充值码" />
            <Button onClick={activate} disabled={activating || !activationCode.trim()}>
              {activating ? '充值中…' : '立即充值'}
            </Button>
          </div>
        </div>
        {activationMsg && <div className={activationMsg.ok ? 'test-ok' : 'test-err'}>{activationMsg.text}</div>}
        {!billing.configured && <div className="form-hint">演示状态：运营端服务器地址将在正式发行包中自动配置。</div>}
      </Card>
