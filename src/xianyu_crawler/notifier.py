...
def format_email_html(events: list[dict]) -> str:
    ...
    return (
        ...
        '<div style="font-size:19px;font-weight:700;color:#0a2540;">寻迹AI助手</div>'
        ...
        '由「寻迹AI助手」自动发送 · 点卡片直达商品</div></div></div>'
    )
...
def send_test(settings) -> None:
    ...
    msg = MIMEText("这是一封来自寻迹AI助手的测试邮件，收到说明 SMTP 配置成功。", "plain", "utf-8")
    msg["Subject"] = Header("寻迹AI助手 · 测试邮件", "utf-8").encode()
