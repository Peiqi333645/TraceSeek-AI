...
def _user_data_dir() -> Path:
    if sys.platform == "darwin":
        return Path.home() / "Library" / "Application Support" / "TraceSeek-AI"
    if sys.platform.startswith("win"):
        base = os.environ.get("APPDATA") or str(Path.home())
        return Path(base) / "TraceSeek-AI"
    return Path.home() / ".traceseek-ai"
...
def _setup_logging(data_dir: Path) -> None:
    ...
    macOS ~/Library/Application Support/TraceSeek-AI/app.log
    """
