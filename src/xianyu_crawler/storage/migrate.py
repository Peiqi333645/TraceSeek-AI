...
def ensure_columns(db_path: str | Path) -> int:
    ...
    try:
        for table, col, ddl in COLUMNS:
            ...
            if table == "watches" and col in {"district", "province"}:
        ...
        if search_logic_upgrade and _cols(con, "events"):
            con.execute("DELETE FROM events WHERE type IN ('new_match', 'new_recommendation')")
        con.commit()
    finally:
        con.close()
    return added
