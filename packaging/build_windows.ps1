...
python packaging/copy_browsers.py "dist/TraceSeek-AI/ms-playwright"

Write-Host ">> 冒烟: 拉起包内 Chromium"
& "dist/TraceSeek-AI/TraceSeek-AI.exe" --selfcheck

Write-Host ">> Inno Setup 打安装包"
& "${env:ProgramFiles(x86)}\Inno Setup 6\ISCC.exe" "/DMyAppVersion=$Version" packaging\traceseek.iss

Write-Host "✓ 完成: packaging\Output\TraceSeek-AI-Setup.exe"
